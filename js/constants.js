...
/** V-23: кайма белого опала на бумаге (--star-opal-edge в .book) — заливка у него
 *  светлая, без каймы на светлом листе она исчезает. */
const PAPER_OPAL_EDGE_RGB = [96, 88, 78];
...
// V-24: у портретных телефонов вьюпорт на минимальном зуме почти всегда круче
// аспектом, чем поле (letterboxing — см. комментарий у ZOOM PARAMETERS ниже),
// и без запаса сверху/снизу оставалась пустая полоса без единой звезды. Запас
// фиксированный (не от текущего вьюпорта): канвас в момент генерации поля
// может быть нерепрезентативным (K-13, встроенный вид экслибриса). Только по
// вертикали — у этого аспекта поля горизонтальный letterbox на портретных
// экранах не возникает.
const BACKGROUND_STAR_MARGIN_Y = 800;
...
// O-10: лента зовёт в книгу, пока её ни разу не открыли. С этого числа созвездий
// за игру (пожизненный totalConstellations, созвездие тутора включительно) зов
// становится жёстким шагом: звёзды неиграбельны, пока не откроешь книгу.
const BOOK_INVITE_GATE_CONSTELLATIONS = 3;

// =============================================================================
// K-20: ГРАВЮРА НА НЕБЕ — капитель имён, рамка пометки (штриховку замкнутой фигуры снял V-21)
// =============================================================================
//
...
// нет вовсе»). Появление и исчезание — темпом MOTION_SCENE_MS (CSS: var(--t-scene)).
// U-36 (правка заказчика): таймера показа больше нет — окно висит, пока игрок
// не тапнет куда угодно, но первые LEVEL_BANNER_MIN_MS тап его не закрывает
// (тот же палец, что забрал награду, не должен смахнуть окно, не дав его увидеть).
const LEVEL_BANNER_MIN_MS = 500;
// U-36: сколько ступеней в цепочке достижений — знаменатель числа «0 / 5» в кружке.
const LEVEL_BANNER_SEAL_STEPS = 5;
// U-36: сторона иконки в строке разблокировки (кружок печати и канвас трафарета).
const LEVEL_BANNER_ICON_PX = 36;
...
const SHAPE_PATTERNS = {
    ...
    // C-08: остриё лезвия смотрит вбок (вправо), рукоять — вниз из нижней левой вершины
    // треугольника; по ориентиру заказчика (поворот всей фигуры целиком не подошёл).
    'spatula': {
        stars: [[0.35, 0.12], [0.85, 0.28], [0.35, 0.45], [0.35, 0.88]],
    ...
    // C-08: нос вверх (было — нос вниз, плавники вверху); чертёж отражён `y → 1 − y`,
    // как и checkmark в C-07 — распознавание от координат не зависит.
    'radish': {
        stars: [[0.5, 0.55], [0.5, 0.28], [0.5, 0.05], [0.3, 0.88], [0.7, 0.88]],
...
const CATALOG_29 = {
    ...
    'spatula': { stars: [[0.35, 0.12], [0.85, 0.28], [0.35, 0.45], [0.35, 0.88]], lines: [[0, 1], [1, 2], [2, 0], [2, 3]], c: { cycleAngleMin: 25, outwardTail: true } },
    ...
    'radish': { stars: [[0.5, 0.55], [0.5, 0.28], [0.5, 0.05], [0.3, 0.88], [0.7, 0.88]], lines: [[0, 1], [1, 2], [0, 3], [0, 4]], c: {} },
    // 5 звёзд — один цикл
