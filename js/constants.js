...
// постоянной кнопки в углу больше нет. Пометка всплывает под подписью только что
// созданной фигуры, называет, что сотрёт, и тает. Окно отмены равно её жизни.
//
...
const UNDO_MARK_TEXT_PX = 13;              // кегль имени в пометке, экранные px
const UNDO_MARK_SIGN_PX = 14;              // знак отмены из кассы K-02 (#i-undo)
const UNDO_MARK_SIGN_GAP_PX = 5;           // между знаком и словом
const UNDO_MARK_DROP_PX = 27;              // от якоря подписи до середины пометки
...
const UNDO_MARK_HIT_PAD_PX = 10;           // зона касания шире надписи, как у ленты K-05
/** K-20: рамка — карточка на бумаге вокруг знака и текста (Табл. II концепта).
 *  Геометрию содержимого по-прежнему даёт computeUndoMarkLayout; рамка рисуется
 *  чуть шире нею на паддинг, зоны касания (UNDO_MARK_HIT_PAD_PX) не касается. */
