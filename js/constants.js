...
const SHAPES = {
    ...
    'Зубочистка':       { color: [225, 225, 210], description: 'Две звезды, одна линия' },
    'Галочка':          { color: [140, 220, 255], description: 'Три звезды цепочкой (уголок)' },
    'Чипсина':          { color: [255, 205, 120], description: 'Треугольник из трёх звёзд' },
    'Печенька':         { color: [210, 170, 120], description: 'Замкнутый четырёхугольник' },
    'Куриная лапка':    { color: [255, 180, 160], description: 'Три луча из одной звезды' },
    'Дождевой червяк':  { color: [180, 230, 170], description: 'Пять звёзд волнистой цепочкой' },
    // Каталог-29 (недемо-фигуры, страницы 2–6)
    'Лопатка':          { color: [200, 220, 160], description: 'Треугольник с ручкой' },
    'Алмазик':          { color: [130, 220, 235], description: 'Четырёхугольник с диагональю' },
    'Конвертик':        { color: [230, 200, 150], description: 'Треугольник со звездой внутри (K4)' },
    'Вентилятор':       { color: [160, 210, 255], description: 'Четыре луча из центра' },
    'Редиска':          { color: [235, 130, 150], description: 'Хвост из двух сегментов и две ветки' },
    'Пончик':           { color: [255, 190, 140], description: 'Замкнутый пятиугольник' },
    'Флажок':           { color: [255, 170, 120], description: 'Четырёхугольник с хвостиком' },
    'Головастик':       { color: [150, 220, 200], description: 'Треугольник с хвостом из двух сегментов' },
    'Зайчик':           { color: [235, 215, 235], description: 'Треугольник с двумя ушами из одной вершины' },
    'Бычок':            { color: [210, 180, 150], description: 'Треугольник с рогами из разных вершин' },
    'Бантик':           { color: [255, 160, 210], description: 'Два треугольника через общую вершину' },
    'Домик':            { color: [220, 190, 150], description: 'Пятиугольник с перекладиной' },
    'Булава':           { color: [200, 200, 215], description: 'Алмазик-навершие с рукоятью' },
    'Воздушный змей':   { color: [170, 220, 255], description: 'Алмазик с хвостом из острия' },
    'Фонарик':          { color: [255, 225, 150], description: 'Два полюса и три перемычки' },
    'Веер':             { color: [255, 200, 200], description: 'Дуга из четырёх и звезда-ручка' },
    'Чупа-чупс':        { color: [255, 150, 190], description: 'K4-конфета с палочкой' },
    'Книжка':           { color: [190, 210, 240], description: 'Три страницы на общем корешке' },
    'Оригами':          { color: [220, 235, 200], description: 'Печенька с центром при трёх углах' },
    'Колесо':           { color: [210, 210, 230], description: 'Печенька с центром при всех углах' },
    'Гамак':            { color: [180, 210, 190], description: 'Подвес сверху и сетка снизу' },
    'Перфекционист':    { color: [255, 235, 180], description: 'Почти полный граф пяти звёзд (K5−1)' },
    // Fallback
...
// S-02: пер-фигурные цепочки завязаны на ЦВЕТ фигуры. У каждого цвета своя
// квота созданий, обратная его частоте (жёлтый — «средний» тир, выпадает чаще
// всего → квота больше; красный/синий — экстремумы спектра, редки → 1).
// Цвет фигуры = ближайший тир к средней colorValue звёзд (constellationColorBucket).
// Цепочка = 5 шагов «собрано цветов»: 1→2→3→4→5. Полный набор = 50 ✦ (10 ✦/цвет).
const SHAPE_COLOR_QUOTAS = { red: 1, orange: 2, yellow: 3, white: 2, blue: 1 };
const SHAPE_CHAIN_STEP_REWARD = 10; // ✦ за каждый собранный цвет (шаг цепочки)
...
const ATLAS_PAGES = [
    ['Галочка', 'Чипсина', 'Печенька'],
    ['Банан', 'Куриная лапка', 'Дождевой червяк'],
    ['Зубочистка', 'Лопатка', 'Алмазик', 'Редиска', 'Вентилятор'],
    ['Пончик', 'Флажок', 'Головастик', 'Зайчик', 'Бычок'],
    ['Бантик', 'Домик', 'Булава', 'Воздушный змей', 'Фонарик'],
    ['Конвертик', 'Веер', 'Чупа-чупс', 'Книжка', 'Оригами'],
    ['Колесо', 'Гамак', 'Перфекционист']
];
const ATLAS_PAGE_COSTS = [40, 100, 180, 300, 450, 650, 900];
...
const SHAPE_XP = {
    ...
    // Каталог-29 (топологический режим)
    'Галочка': 10, 'Чипсина': 10, 'Печенька': 10, 'Куриная лапка': 10,
    'Дождевой червяк': 10, 'Зубочистка': 10,
    // Каталог-29 (недемо-фигуры, страницы 2–6)
    'Лопатка': 10, 'Алмазик': 10, 'Конвертик': 10, 'Вентилятор': 10, 'Редиска': 10,
    'Пончик': 10, 'Флажок': 10, 'Головастик': 10, 'Зайчик': 10, 'Бычок': 10,
    'Бантик': 10, 'Домик': 10, 'Булава': 10, 'Воздушный змей': 10, 'Фонарик': 10,
    'Веер': 10, 'Чупа-чупс': 10, 'Книжка': 10, 'Оригами': 10,
    'Колесо': 10, 'Гамак': 10, 'Перфекционист': 10,
    'Фигура': 10
};
...
const ALL_SHAPE_NAMES = [
    ...
    'Галочка', 'Чипсина', 'Печенька', 'Куриная лапка', 'Дождевой червяк', 'Зубочистка',
    // Каталог-29 (недемо-фигуры, страницы 2–6)
    'Лопатка', 'Алмазик', 'Конвертик', 'Вентилятор', 'Редиска',
    'Пончик', 'Флажок', 'Головастик', 'Зайчик', 'Бычок',
    'Бантик', 'Домик', 'Булава', 'Воздушный змей', 'Фонарик',
    'Веер', 'Чупа-чупс', 'Книжка', 'Оригами',
    'Колесо', 'Гамак', 'Перфекционист',
    'Фигура'
];
...
const FALLBACK_NAMES = [
    ...
    'Брызги', 'Флюктуация', 'Переливы', 'Узор', 'Вязь',
...
const CATALOG_29 = {
    // 2 звезды
    'Зубочистка': { stars: [[0.15, 0.5], [0.85, 0.5]], lines: [[0, 1]], c: {} },
    // 3 звезды
    'Галочка': { stars: [[0.15, 0.3], [0.5, 0.7], [0.85, 0.3]], lines: [[0, 1], [1, 2]], c: { chainAngle: [25, 155] } },
    'Чипсина': { stars: [[0.5, 0.15], [0.15, 0.8], [0.85, 0.8]], lines: [[0, 1], [1, 2], [2, 0]], c: { cycleAngleMin: 25 } },
    // 4 звезды
    'Банан': { stars: [[0.08, 0.38], [0.37, 0.62], [0.63, 0.62], [0.92, 0.38]], lines: [[0, 1], [1, 2], [2, 3]], c: { arc: true, chainAngle: [100, 170] } },
    'Куриная лапка': { stars: [[0.5, 0.65], [0.2, 0.2], [0.5, 0.15], [0.8, 0.2]], lines: [[0, 1], [0, 2], [0, 3]], c: {} },
    'Печенька': { stars: [[0.2, 0.2], [0.8, 0.2], [0.8, 0.8], [0.2, 0.8]], lines: [[0, 1], [1, 2], [2, 3], [3, 0]], c: { convex: true } },
    'Лопатка': { stars: [[0.35, 0.2], [0.65, 0.2], [0.5, 0.45], [0.5, 0.85]], lines: [[0, 1], [1, 2], [2, 0], [2, 3]], c: { cycleAngleMin: 25, outwardTail: true } },
    'Алмазик': { stars: [[0.5, 0.1], [0.8, 0.5], [0.5, 0.9], [0.2, 0.5]], lines: [[0, 1], [1, 2], [2, 3], [3, 0], [1, 3]], c: { convex: true } },
    'Конвертик': { stars: [[0.5, 0.1], [0.1, 0.85], [0.9, 0.85], [0.5, 0.55]], lines: [[0, 1], [1, 2], [2, 0], [3, 0], [3, 1], [3, 2]], c: { cycleAngleMin: 25, centerInside: true } },
    // 5 звёзд — деревья
    'Дождевой червяк': { stars: [[0.06, 0.5], [0.28, 0.35], [0.5, 0.6], [0.72, 0.35], [0.94, 0.5]], lines: [[0, 1], [1, 2], [2, 3], [3, 4]], c: { chainAngle: [80, 180] } },
    'Вентилятор': { stars: [[0.5, 0.5], [0.5, 0.08], [0.92, 0.5], [0.5, 0.92], [0.08, 0.5]], lines: [[0, 1], [0, 2], [0, 3], [0, 4]], c: { spreadMin: 45 } },
    'Редиска': { stars: [[0.5, 0.45], [0.5, 0.72], [0.5, 0.95], [0.3, 0.12], [0.7, 0.12]], lines: [[0, 1], [1, 2], [0, 3], [0, 4]], c: {} },
    // 5 звёзд — один цикл
    'Пончик': { stars: [[0.5, 0.1], [0.89, 0.38], [0.74, 0.88], [0.26, 0.88], [0.11, 0.38]], lines: [[0, 1], [1, 2], [2, 3], [3, 4], [4, 0]], c: {} },
    'Флажок': { stars: [[0.3, 0.12], [0.7, 0.12], [0.7, 0.5], [0.3, 0.5], [0.7, 0.92]], lines: [[0, 1], [1, 2], [2, 3], [3, 0], [2, 4]], c: { convex: true, outwardTail: true } },
    'Головастик': { stars: [[0.3, 0.15], [0.6, 0.1], [0.5, 0.35], [0.62, 0.62], [0.8, 0.85]], lines: [[0, 1], [1, 2], [2, 0], [2, 3], [3, 4]], c: { cycleAngleMin: 25, outwardTail: true } },
    'Зайчик': { stars: [[0.5, 0.45], [0.3, 0.8], [0.7, 0.8], [0.3, 0.1], [0.7, 0.1]], lines: [[0, 1], [1, 2], [2, 0], [0, 3], [0, 4]], c: { earsBetween: [20, 80] } },
    'Бычок': { stars: [[0.25, 0.4], [0.75, 0.4], [0.5, 0.8], [0.12, 0.1], [0.88, 0.1]], lines: [[0, 1], [1, 2], [2, 0], [0, 3], [1, 4]], c: {} },
    // 5 звёзд — два цикла
    'Бантик': { stars: [[0.5, 0.5], [0.1, 0.3], [0.1, 0.7], [0.9, 0.3], [0.9, 0.7]], lines: [[0, 1], [1, 2], [2, 0], [0, 3], [3, 4], [4, 0]], c: {} },
    'Домик': { stars: [[0.25, 0.9], [0.75, 0.9], [0.75, 0.45], [0.5, 0.12], [0.25, 0.45]], lines: [[0, 1], [1, 2], [2, 3], [3, 4], [4, 0], [2, 4]], c: {} },
    'Булава': { stars: [[0.5, 0.08], [0.78, 0.45], [0.5, 0.8], [0.22, 0.45], [0.78, 0.9]], lines: [[0, 1], [1, 2], [2, 3], [3, 0], [1, 3], [1, 4]], c: { convex: true, outwardTail: true } },
    'Воздушный змей': { stars: [[0.5, 0.06], [0.72, 0.32], [0.5, 0.58], [0.28, 0.32], [0.5, 0.94]], lines: [[0, 1], [1, 2], [2, 3], [3, 0], [1, 3], [2, 4]], c: { convex: true, outwardTail: true } },
    'Фонарик': { stars: [[0.5, 0.08], [0.5, 0.92], [0.15, 0.5], [0.5, 0.5], [0.85, 0.5]], lines: [[0, 2], [0, 3], [0, 4], [1, 2], [1, 3], [1, 4]], c: {} },
    // 5 звёзд — плотные
    'Веер': { stars: [[0.1, 0.3], [0.37, 0.15], [0.63, 0.15], [0.9, 0.3], [0.5, 0.85]], lines: [[0, 1], [1, 2], [2, 3], [4, 0], [4, 1], [4, 2], [4, 3]], c: { fanMax: 180 } },
    'Чупа-чупс': { stars: [[0.25, 0.15], [0.75, 0.15], [0.5, 0.55], [0.5, 0.28], [0.5, 0.92]], lines: [[0, 1], [1, 2], [2, 0], [3, 0], [3, 1], [3, 2], [2, 4]], c: { cycleAngleMin: 25, centerInside: true, outwardTail: true } },
    'Книжка': { stars: [[0.3, 0.15], [0.3, 0.85], [0.55, 0.5], [0.72, 0.5], [0.92, 0.5]], lines: [[0, 1], [0, 2], [1, 2], [0, 3], [1, 3], [0, 4], [1, 4]], c: { baseMin: 5 } },
    'Оригами': { stars: [[0.2, 0.2], [0.8, 0.2], [0.8, 0.8], [0.2, 0.8], [0.5, 0.5]], lines: [[0, 1], [1, 2], [2, 3], [3, 0], [4, 1], [4, 2], [4, 3]], c: { convex: true, centerInside: true } },
    'Колесо': { stars: [[0.2, 0.2], [0.8, 0.2], [0.8, 0.8], [0.2, 0.8], [0.5, 0.5]], lines: [[0, 1], [1, 2], [2, 3], [3, 0], [4, 0], [4, 1], [4, 2], [4, 3]], c: { convex: true, centerInside: true } },
    'Гамак': { stars: [[0.5, 0.04], [0.18, 0.35], [0.82, 0.35], [0.5, 0.92], [0.5, 0.55]], lines: [[0, 1], [0, 2], [1, 2], [1, 3], [2, 3], [1, 4], [2, 4], [3, 4]], c: {} },
    'Перфекционист': { stars: [[0.5, 0.2], [0.1, 0.75], [0.9, 0.75], [0.5, 0.55], [0.5, 0.02]], lines: [[0, 1], [0, 2], [1, 2], [3, 0], [3, 1], [3, 2], [4, 0], [4, 1], [4, 2]], c: { baseMin: 5 } }
};
