...
// B-01 растянул пороги ×8 от исходных [1,5,15,30,60], чтобы пул не выгорал за
// месяц. Правка сверху: первые две ступени возвращены к исходным — цепочка должна
// подавать признаки жизни с первых ночей, иначе новый игрок не видит, что она
// вообще движется. Верхние три остаются расчётными: смягчение и их тоже ломало
// метроном — доход рос сильнее у того, кто делает больше созвездий, и разброс
// между профилями уезжал с 1,24× до 1,51×.
...
const ACHIEVEMENT_VOLUME_TIERS = [1, 5, 120, 240, 480];
...
const ACHIEVEMENT_CHAINS = [
    ...
    ...[3, 4, 5, 6, 7].map(buildExactSizeChain),
    {
        ...
        // исходные [1,3], верхние три — расчётные ×8 от прежних [8,15,25].
        steps: [1, 3, 64, 120, 200].map(n => ({
            id: `size_8plus_${n}`,
